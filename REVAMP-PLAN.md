...
| **2 — Content & narrative** 🟡 | All pages live with real copy | **Done:** `/services` (Build/Advise/Products), `/work` + case studies (Clip/NewsNook/journalism), `/about` (+patents), `/products/newsnook`, `/contact` (intent-routed + route handler), `/faq` (+FAQPage JSON-LD). **Remaining:** migrate `/patents` + `/team` and port legal (URL parity), Verbaly case study, full Turnstile + booking wiring. Case studies use typed content (`lib/content.ts`); MDX deferred. |
